        ...
        roles[0].name == "SELLER" ?
          (<Paper square elevation={3} sx={{ width: "70%", height: "50%", paddingBottom: "2.5%", pl: "2.5%", pr: "2.5%", margin: "auto", mt: "2.5%", display: "flex", flexDirection: "column", overflowY: "scroll" }}>
            {products.length > 0 && products.map(product => {
              return (<div key={product.objectID} style={{ marginTop: "3.5%" }}><MyProductCard key={product.objectID} objectID={product.objectID} name={product.name} price={product.price} quantity={product.quantity} categories={product.categories} description={product.description} /></div>);
            })}
